...
class userController extends Controller
{
    ...
    public function post($id,orderRequest $r)
    {
        ...
        {
            $r->session()->put('orderCounter',1);
            $c=$id.":".$r->quantity.":".$r->color.":".$r->session()->get('orderCounter');   //the order counter is added after color so that order serial can be obtained
            $r->session()->put('cart',$c);
        }
        else
        {
            $r->session()->put('orderCounter',$r->session()->get('orderCounter')+1);
            $cd=$id.":".$r->quantity.":".$r->color.":".$r->session()->get('orderCounter');
            $total=$r->session()->get('cart').",".$cd;
    ...
    {   //$r->session()->forget('cart');
        ...
        $totalCart = explode(',',$r->session()->get('cart'));
    	...
    	return view('store.cart')
            ...
            ->with('all',$cart)
            ->with('prod',$product)
            ->with('total',$r->session()->get('price'));
    }
...
//    for quntity control in cart
    ...
    {    
        ...
        $newproduct=[];
        $newcost=0;
        $newtotalCart = explode(',',$r->session()->get('cart'));
        //dd($r->session()->get('cart'));
        ...
        foreach($newcart as $t)
        {
                ...
                if(!($r->session()->has('tempCart')))
                {

                    $str=$t[0].":".$t[1].":".$t[2].":".$t[3];
                    $r->session()->put('tempCart',$str);
                }
                else
                {
                    $str2=$t[0].":".$t[1].":".$t[2].":".$t[3];
                    $mytotal=$r->session()->get('tempCart').",".$str2;
                    $r->session()->put('tempCart',$mytotal);
                }
                
        }
            $r->session()->forget('cart');
            $r->session()->put('cart',$r->session()->get('tempCart'));
            $r->session()->forget('tempCart');
            ...
            //dd($r->session()->get('price'));
            //end 
            //dd($myarr);
            $szn[0]=$r->session()->get('cart');
            $szn[1]=$r->session()->get('price');
            $szn[2]=$cost;
...
//    for quntity control in cart ENDS
    ...
    public function deleteCartItem(Request $r)
    {
            ...
            foreach($newcart2 as $t2)
        {
                ...
                {
                    $str2=$t2[0].":".$t2[1].":".$t2[2].":".$t2[3];
                    $mytotal2=$r->session()->get('tempCart').",".$str2;
                    $r->session()->put('tempCart',$mytotal2);
                }
